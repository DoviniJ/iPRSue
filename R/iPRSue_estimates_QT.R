...
#' for quantitative traits. This implementation follows the iPRSue framework, simulating multiple PRSs by sampling from
#' the GWAS effect size distribution and deriving individual-level confidence intervals.
#'
#' @param gwas A data frame with GWAS summary statistics for a quantitative trait. Must contain `beta` and `se` columns representing
...
#'
#' @export
iPRSue_estimates_QT <- function(gwas, target_pheno,
                                target_geno_mat, 
                                no_of_PRSs = 500, 
                                significance_level = 0.05,
                                seed = NULL){
