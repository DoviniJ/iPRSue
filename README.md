...
## $\color{red} {IMPORTANT}$
When individual level data to use as a discovery sample are unavailable, users can often chose to utilize publicly available summary statistics data. However, these publicly available GWAS summary statistics often include only the \(p\)-value. In such cases, one can derive the Z-statistic ($Z_i$) assuming a standard normal distribution, and then compute the standard error as $\hat\sigma_i^{(\mathrm{scaled})} = 1/\sqrt{n}$ and effect size as $\hat\beta_i^{(\mathrm{scaled})} = Z_i/\sqrt{n}$, where $n$ corresponds to the discovery sample size. Once both are obtained, the iPRSue method can be applied using either ```iPRSue_estimates_QT()``` or ```iPRSue_estimates_BT()``` functions accordingly.
